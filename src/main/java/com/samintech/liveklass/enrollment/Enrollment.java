 ...
 * 신청 상태(PENDING, CONFIRMED, CANCELLED) 및 관련 비즈니스 규칙(취소 기한 등)을 캡슐화합니다.
 */
@Entity
@Table(
    name = "enrollments",
...
public class Enrollment {
    ...
    /**
     * 대기열(WAITLISTED) 상태의 수강 신청을 결제 대기(PENDING)로 승격합니다.
     * 앞선 수강생이 취소하여 자리가 생겼을 때 호출됩니다.
     */
    public void promote() {
    ...
    /**
     * 수강 신청 상태를 '결제 완료(CONFIRMED)'로 변경합니다.
     * 비즈니스 규칙: 수강 신청은 PENDING(결제 대기) 상태에서만 CONFIRMED로 변경할 수 있습니다.
     */
    public void confirm() {
     ...
     * 수강 신청을 취소합니다.
     * 비즈니스 규칙:
     * - 이미 취소된 상태(CANCELLED)라면 중복 취소가 불가능합니다.
     * - 결제 확정(CONFIRMED) 상태인 경우, 설정된 취소 가능 기한(cancelWindowDays) 이내에만 취소할 수 있습니다.
     * - PENDING 상태는 기한 제약 없이 언제든 취소 가능합니다.
     *
